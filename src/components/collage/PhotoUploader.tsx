// src/components/collage/PhotoUploader.tsx - FIXED WITH COMPRESSION & TRANSPARENCY
import React, { useState, useRef } from 'react';
import { Upload, X, Check, AlertCircle, RefreshCw, Image, FileImage } from 'lucide-react';
import { compressImage, COMPRESSION_PRESETS, formatFileSize } from '../../utils/imageCompression';
...
const PhotoUploader: React.FC<PhotoUploaderProps> = ({ collageId, onUploadComplete }) => {
  ...
  const handleFileSelect = async (files: File[]) => {
    ...
    processCompressedUploads(newUploads);
  };
  ...
  // Process uploads with compression
  const processCompressedUploads = async (uploads: FileUpload[]) => {
    setIsUploading(true);
      ...
      const uploadPromises = batch.map(async (upload) => {
        try {
          ...
          const progressInterval = setInterval(() => {
            setFileUploads(prev => prev.map(u => 
              u.id === upload.id && u.status === 'uploading'
                ? { ...u, progress: Math.min(u.progress + 10, 40) }
                : u
            ));
          }, 300);
          ...
          updateFileStatus(upload.id, { 
            status: 'uploading', 
            progress: 60,
            error: `Compressed: ${formatFileSize(compressionResult.compressedSize)} (${compressionResult.compressionRatio.toFixed(0)}% smaller)`
          });

          // Upload the compressed photo
          await uploadPhoto(collageId, compressionResult.file);
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div className="text-xs text-gray-500">
            Supports JPEG, PNG, GIF, WebP • Max 10MB per file • Auto-compressed for faster uploads
          </div>
        </div>
      </div>
            ...
            {fileUploads.map((upload) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {upload.status === 'uploading' && upload.error && upload.error.includes('Compressed') && (
                    <p className="text-blue-400 text-xs mt-1">{upload.error}</p>
                  )}

                  {upload.status === 'success' && (
                    <p className="text-green-400 text-xs mt-1">
                      Upload complete • {formatFileSize(upload.file.size)}
                    </p>
                  )}
                </div>
      ...
      <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4">
        ...
        <ul className="text-blue-200 text-sm space-y-1">
          <li>• Photos are automatically compressed for faster uploads</li>
          <li>• PNG/GIF transparency is preserved automatically</li>
          <li>• Images appear in the collage instantly after upload</li>
          <li>• Best quality: Use high-resolution images (1080p or higher)</li>
          <li>• Supported formats: JPEG, PNG, GIF, WebP</li>
          <li>• Maximum file size: 10MB per photo</li>
          <li>• Upload multiple photos at once for batch processing</li>
        </ul>
      </div>
    </div>
  );
