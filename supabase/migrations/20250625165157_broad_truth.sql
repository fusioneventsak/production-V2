/*
  # Fix Realtime Publication for Photo Deletion Events
  
  1. Changes
    - Ensures the supabase_realtime publication exists and includes DELETE events
    - Adds diagnostic functions to check and test realtime functionality
    - Properly handles existing functions to avoid errors
  
  2. Benefits
    - Ensures photo deletion events are properly broadcast to clients
    - Provides tools to diagnose realtime issues
    - Improves reliability of the realtime system
*/
...
DROP FUNCTION IF EXISTS test_realtime(uuid);
